...
<Details>
<Summary>
How do I create a theme switcher or color mode toggle button?
</Summary>
<Body>

Use `IClientProvider.SetThemeMode()` to switch between light, dark, and system themes:
...
public class ThemeSwitcher : ViewBase
{
    public override object? Build()
    {
        ...
        return Layout.Vertical()
            | new Button("Light").OnClick(() => client.SetThemeMode(ThemeMode.Light))
            | new Button("Dark").OnClick(() => client.SetThemeMode(ThemeMode.Dark))
            | new Button("System").OnClick(() => client.SetThemeMode(ThemeMode.System));
    }
}
```
...
<Details>
<Summary>
How do I toggle between dark and light mode?
</Summary>
<Body>

Use `IClientProvider.SetThemeMode()` with `ThemeMode.Light` or `ThemeMode.Dark`. See the theme switcher example above.
