...
[App(order:2, documentSource:"https://github.com/Ivy-Interactive/Ivy-Framework/blob/2587-rename-appattribute-path-parameter-to-group/src/Ivy.Docs.Shared/Docs/01_Onboarding/03_CLI/04_Authentication/02_Auth0.md", searchHints: ["auth0", "authentication", "oauth", "social-login", "identity", "sso"])]
public class Auth0App(bool onlyBody = false) : ViewBase
{
    public Auth0App() : this(false)
    {
    }
    public override object? Build()
    {
        ...
        var article = new Article().ShowToc(!onlyBody).ShowFooter(!onlyBody).Previous(appDescriptor.Previous).Next(appDescriptor.Next).DocumentSource(appDescriptor.DocumentSource).OnLinkClick(onLinkClick).Headings(new List<ArticleHeading> { new ArticleHeading("auth0-authentication-provider", "Auth0 Authentication Provider", 1), new ArticleHeading("overview", "Overview", 2), new ArticleHeading("getting-your-auth0-configuration", "Getting Your Auth0 Configuration", 2), new ArticleHeading("step-1-create-an-auth0-account-and-application", "Step 1: Create an Auth0 Account and Application", 3), new ArticleHeading("step-2-configure-your-application", "Step 2: Configure Your Application", 3), new ArticleHeading("step-3-get-your-configuration-values", "Step 3: Get Your Configuration Values", 3), new ArticleHeading("step-4-create-an-api", "Step 4: Create an API", 3), new ArticleHeading("step-5-enable-authentication-options", "Step 5: Enable Authentication Options", 3), new ArticleHeading("email-and-password", "Email and Password", 4), new ArticleHeading("adding-users-for-email-and-password-authentication", "Adding Users for Email and Password Authentication", 5), new ArticleHeading("google", "Google", 4), new ArticleHeading("github", "GitHub", 4), new ArticleHeading("microsoft", "Microsoft", 4), new ArticleHeading("apple", "Apple", 4), new ArticleHeading("twitterx", "Twitter/X", 4), new ArticleHeading("step-6-add-claims-to-jwt", "Step 6: Add claims to JWT", 3), new ArticleHeading("adding-authentication", "Adding Authentication", 2), new ArticleHeading("advanced-configuration", "Advanced Configuration", 3), new ArticleHeading("connection-strings", "Connection Strings", 4), new ArticleHeading("manual-configuration", "Manual Configuration", 4), new ArticleHeading("configuration-parameters", "Configuration Parameters", 4), new ArticleHeading("authentication-flow", "Authentication Flow", 2), new ArticleHeading("emailpassword-flow", "Email/Password Flow", 3), new ArticleHeading("social-login-flow", "Social Login Flow", 3), new ArticleHeading("auth0-specific-features", "Auth0-Specific Features", 2), new ArticleHeading("security-best-practices", "Security Best Practices", 2), new ArticleHeading("troubleshooting", "Troubleshooting", 2), new ArticleHeading("common-issues", "Common Issues", 3), new ArticleHeading("related-documentation", "Related Documentation", 2), })
            | new global::Ivy.Docs.Shared.Internal.SmartSearchView()
            | new Markdown("# Auth0 Authentication Provider").OnLinkClick(onLinkClick)
            | Lead("Secure your Ivy application with Auth0's universal authentication platform supporting multiple identity providers and social logins.")
            | new Markdown(
                """"
                ## Overview
                ...
                2. **Set Allowed Callback URLs** to `http://localhost:5010/ivy/auth/callback`, replacing the base URL (`http://localhost:5010`) with your application's URL: ![Auth0 Allowed Callback URLs](/ivy/assets/auth0_callback.webp "Auth0 Allowed Callback URLs")
                3. **Click "Save"**
            ...
            | new Markdown(
                ...
                Your credentials will be stored securely in [.NET user secrets](app://onboarding/concepts/secrets). You will then be prompted to choose one or more authentication options to support, from the following list:
                ...
                2. Dynamically generates and adds the appropriate `UseAuth<Auth0AuthProvider>()` call to your [Program.cs](app://onboarding/concepts/program) based on your selected options (e.g., if you select Email/Password and Google, it generates: `server.UseAuth<Auth0AuthProvider>(c => c.UseEmailPassword().UseGoogle());`).
                ...
                """").OnLinkClick(onLinkClick)
            | new Terminal()
                .AddCommand("ivy auth add --provider Auth0 --connection-string \"Auth0:Domain=your-domain.auth0.com;Auth0:ClientId=your-client-id;Auth0:ClientSecret=your-client-secret;Auth0:Namespace=https://your-domain.com/\"")
        ...
        Type[] _ = [typeof(Onboarding.Concepts.SecretsApp), typeof(Onboarding.Concepts.ProgramApp), typeof(Onboarding.CLI.Authentication.AuthenticationOverviewApp), typeof(Onboarding.CLI.Authentication.SupabaseApp), typeof(Onboarding.CLI.Authentication.MicrosoftEntraApp)]; 
        return article;
    }
}
