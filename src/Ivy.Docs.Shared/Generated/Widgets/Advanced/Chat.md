...
[View Source: Chat.cs](https://github.com/Ivy-Interactive/Ivy-Framework/blob/main/src/Ivy/Widgets/Chat.cs)

### Constructors

| Signature |
|-----------|
| `new Chat(ChatMessage[] messages, Func<Event<Chat, string>, ValueTask> onSend, Func<Event<Chat>, ValueTask> onCancel = null)` |
| `new Chat(ChatMessage[] messages, Action<Event<Chat, string>> onSend)` |
| `new Chat(ChatMessage[] messages, Action<Event<Chat, string>> onSend, Action<Event<Chat>> onCancel)` |
...
| Name | Type | Handlers |
|------|------|----------|
| `OnCancel` | `EventHandler<Event<Chat>>` | - |
| `OnSend` | `EventHandler<Event<Chat, string>>` | - |