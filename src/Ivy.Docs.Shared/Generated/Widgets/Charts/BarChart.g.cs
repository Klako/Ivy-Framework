...
[App(order:2, documentSource:"https://github.com/Ivy-Interactive/Ivy-Framework/blob/2587-rename-appattribute-path-parameter-to-group/src/Ivy.Docs.Shared/Docs/02_Widgets/06_Charts/02_BarChart.md", searchHints: ["visualization", "graph", "analytics", "data", "comparison", "statistics"])]
public class BarChartApp(bool onlyBody = false) : ViewBase
{
    public BarChartApp() : this(false)
    {
    }
    public override object? Build()
    {
        ...
        var article = new Article().ShowToc(!onlyBody).ShowFooter(!onlyBody).Previous(appDescriptor.Previous).Next(appDescriptor.Next).DocumentSource(appDescriptor.DocumentSource).OnLinkClick(onLinkClick).Headings(new List<ArticleHeading> { new ArticleHeading("barchart", "BarChart", 1), new ArticleHeading("filling-with-custom-colors", "Filling with custom colors", 2), new ArticleHeading("api", "API", 2), new ArticleHeading("examples", "Examples", 2), })
            | new global::Ivy.Docs.Shared.Internal.SmartSearchView()
            | new Markdown("# BarChart").OnLinkClick(onLinkClick)
            | Lead("Compare values across categories with rectangular bars of proportional heights.")
            | new Markdown(
                """"
                `BarChart`s compare values across categories. Build chart [views](app://onboarding/concepts/views) inside [layouts](app://onboarding/concepts/layout) and use [state](app://hooks/core/use-state) for dynamic data. See [Charts](app://onboarding/concepts/charts) for an overview of Ivy chart widgets. The sample below shows a stacked bar
            ...
            | (Vertical() 
                | new CodeBlock(
                    ...
                    """",Languages.Csharp)
                | new Box().Content(new BarChartBasic())
            )
            | new Markdown(
                """"
                ...
                Here instead of using a preset `ColorScheme`, a particular bar can also be filled using a custom [Colors](app://api-reference/ivy/colors) value.
                """").OnLinkClick(onLinkClick)
            | (Vertical() 
                | new CodeBlock(
                    """"
                    ...
                    """",Languages.Csharp)
                | new Box().Content(new RainbowBarChartBasic())
            )
            | new Markdown(
                """"
                There are several functions used in this example. `Fill` is used to fill a bar chart
                with a specific [Colors](app://api-reference/ivy/colors) value. The `LegendType` function is used to configure the legend
                to use squares. Using the `Name` function, the name of a bar can be renamed. Like
                here is done for the `Blueberry` column.
                """").OnLinkClick(onLinkClick)
            | new WidgetDocsView("Ivy.BarChart", "Ivy.BarChartExtensions", "https://github.com/Ivy-Interactive/Ivy-Framework/blob/main/src/Ivy/Widgets/Charts/BarChart.cs")
            | new Markdown("## Examples").OnLinkClick(onLinkClick)
            | new Expandable("TIOBE Programming Language Rankings Example",
                Vertical().Gap(4)
                | new Markdown("BarChart handles negative and double values. Changing the value of the year, changes the chart below.").OnLinkClick(onLinkClick)
                | Tabs( 
                    new Tab("Demo", new Box().Content(new TiobeIndexDemo())),
        ...
        Type[] _ = [typeof(Onboarding.Concepts.ViewsApp), typeof(Onboarding.Concepts.LayoutApp), typeof(Hooks.Core.UseStateApp), typeof(Onboarding.Concepts.ChartsApp), typeof(ApiReference.Ivy.ColorsApp)]; 
        return article;
    }
}
