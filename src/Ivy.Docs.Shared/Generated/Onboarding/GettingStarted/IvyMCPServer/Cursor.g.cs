...
[App(order:3, documentSource:"https://github.com/Ivy-Interactive/Ivy-Framework/blob/2587-rename-appattribute-path-parameter-to-group/src/Ivy.Docs.Shared/Docs/01_Onboarding/01_GettingStarted/07_IvyMCPServer/03_Cursor.md")]
public class CursorApp(bool onlyBody = false) : ViewBase
{
    public CursorApp() : this(false)
    {
    }
    public override object? Build()
    {
        ...
        var article = new Article().ShowToc(!onlyBody).ShowFooter(!onlyBody).Previous(appDescriptor.Previous).Next(appDescriptor.Next).DocumentSource(appDescriptor.DocumentSource).OnLinkClick(onLinkClick).Headings(new List<ArticleHeading> { new ArticleHeading("getting-started-cursor", "Getting Started: Cursor", 1), new ArticleHeading("setup", "Setup", 2), })
            | new global::Ivy.Docs.Shared.Internal.SmartSearchView()
            | new Markdown("# Getting Started: Cursor").OnLinkClick(onLinkClick)
            ...
            | new Terminal()
                .AddOutput("ivy init --hello --cursor")
