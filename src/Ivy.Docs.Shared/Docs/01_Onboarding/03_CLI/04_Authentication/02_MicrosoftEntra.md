...
![Entra client and tenant IDs](/ivy/ivy-assets/entra_client_and_tenant_ids.webp "Entra client and tenant IDs")
...
![Newly-created Entra client secret](/ivy/ivy-assets/entra_client_secret.webp "Newly-created Entra client secret")
...
![ID tokens enabled](/ivy/ivy-assets/entra_id_tokens_enabled.webp "ID tokens enabled")
...
When deploying an Ivy project without using `ivy deploy`, your local [.NET user secrets](../../02_Concepts/14_Secrets.md) are not automatically transferred. In that case, you can configure Entra Auth by setting environment variables or .NET user secrets. See Configuration Parameters below.
