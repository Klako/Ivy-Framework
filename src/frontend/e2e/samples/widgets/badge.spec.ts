...
const ICON_TYPES = ["With Bell", "With Heart", "With Star", "With Check"] as const;
...
async function setupBadgePage(page: Page): Promise<void> {
  ...
  const firstResult = page.locator("button").filter({ hasText: /Badge/i }).first();
...
test.describe("Badge Widget Tests", () => {
  ...
  test.describe("All States and Variants", () => {
    ...
    test("should verify all sizes with correct dimensions", async ({ page }) => {
    ...
    test("should verify icon-only badges are smaller than text badges", async ({ page }) => {
  ...
  test.describe("Visual Properties", () => {
    ...
    test("should have different colors for different variants", async ({ page }) => {
      const testVariants = [BADGE_VARIANTS[0], BADGE_VARIANTS[1], BADGE_VARIANTS[4]];
      const badges = await Promise.all(testVariants.map((v) => getBadgeLocator(page, v).first()));

      const colors = await Promise.all(
        badges.map((b) => b.evaluate((el) => window.getComputedStyle(el).backgroundColor)),
      );
  ...
  test.describe("Complex Scenarios", () => {
    test("should verify combined size and icon properties", async ({ page }) => {
    ...
    test("should have proper layout with multiple badges visible", async ({ page }) => {
