...
[App(order:5, documentSource:"https://github.com/Ivy-Interactive/Ivy-Framework/blob/2587-rename-appattribute-path-parameter-to-group/src/Ivy.Docs.Shared/Docs/01_Onboarding/01_GettingStarted/07_IvyMCPServer/05_Antigravity.md")]
public class AntigravityApp(bool onlyBody = false) : ViewBase
{
    public AntigravityApp() : this(false)
    {
    }
    public override object? Build()
    {
        ...
        var article = new Article().ShowToc(!onlyBody).ShowFooter(!onlyBody).Previous(appDescriptor.Previous).Next(appDescriptor.Next).DocumentSource(appDescriptor.DocumentSource).OnLinkClick(onLinkClick).Headings(new List<ArticleHeading> { new ArticleHeading("getting-started-antigravity", "Getting Started: Antigravity", 1), new ArticleHeading("setup", "Setup", 2), new ArticleHeading("manual-configuration", "Manual Configuration", 3), })
            | new global::Ivy.Docs.Shared.Internal.SmartSearchView()
            | new Markdown("# Getting Started: Antigravity").OnLinkClick(onLinkClick)
