...
export function useDrag(
  ...
  eventHandlerRef: React.MutableRefObject<(name: string, id: string, args: unknown[]) => void>,
    ...
    (event: { active: { id: string | number }; over: { id: string | number } | null }) => {
      ...
      if (active && over && active.id !== over.id) {
        ...
        const reorderMapping = newOrder.map((tabId) => {
          ...
          if (index === -1) {
            console.error("Tab reorder error: Tab ID not found in original order", {
              tabId,
              newOrder,
              originalTabOrder,
            });
        ...
        if (!reorderMapping.includes(-1) && reorderMapping.length === originalTabOrder.length) {
  ...
  React.useEffect(() => {
    const handleTabEvent = (eventType: string) => (e: Event) => {
      ...
      if (idx !== -1 && events?.includes(eventType)) eventHandlerRef.current(eventType, id, [idx]);
    };
