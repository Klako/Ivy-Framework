...
const getCardByTestId = (page: Page, testId: string) => page.getByTestId(testId);
...
async function setupCardPage(page: Page): Promise<void> {
  ...
  const firstResult = page.locator("button").filter({ hasText: /Card/i }).first();
  await firstResult.click();
  await page.waitForLoadState("networkidle");
}

// Helper to get computed style property
async function getComputedStyleProperty(element: Locator, property: string): Promise<string> {
...
async function verifyMultipleCards(page: Page, testIds: string[]): Promise<void> {
...
test.describe("Card Widget Tests", () => {
  ...
  test.describe("Smoke Tests", () => {
    test("should render card app with heading and multiple cards", async ({ page }) => {
      await expect(page.getByRole("heading", { level: 1 })).toBeVisible();
      expect(await getCardByRole(page).count()).toBeGreaterThanOrEqual(3);
    });

    test("should render cards with test ids and correct role", async ({ page }) => {
      const testIds = ["card-app", "card-border", "card-border-color"];
      await verifyMultipleCards(page, testIds);
    });

    test("should render cards with icons and progress bars", async ({ page }) => {
      ...
      await expect(salesCard.locator('div[role="progressbar"]').first()).toBeVisible();
    });
  });

  test.describe("State and Property Tests", () => {
    test("should render cards with different border property variations", async ({ page }) => {
    ...
    test("should render cards with optional properties present and absent", async ({ page }) => {
      ...
      await expect(getCardByTestId(page, "card-app-signup-button")).toBeVisible();
  ...
  test.describe("Interactive Behavior and State Updates", () => {
    test("should handle card and button click events with state persistence", async ({ page }) => {
    ...
    test("should maintain all properties after interactions", async ({ page }) => {
  ...
  test.describe("Complex Layout Tests", () => {
    test("should render cards with nested layouts and complex content", async ({ page }) => {
      const salesCard = getCardByTestId(page, "card-total-sales");
      await salesCard.scrollIntoViewIfNeeded();
      await expect(salesCard).toBeVisible();
  ...
  test.describe("Method Verification", () => {
    test("should verify all card methods render correctly", async ({ page }) => {
      const borderCard = getCardByTestId(page, "card-border");
      await borderCard.scrollIntoViewIfNeeded();
      ...
      const borderRadius = await getComputedStyleProperty(borderCard, "borderRadius");
      expect(borderRadius).not.toBe("0px");
    ...
    test("should verify method state updates persist through interactions", async ({ page }) => {
      const testCards = [
