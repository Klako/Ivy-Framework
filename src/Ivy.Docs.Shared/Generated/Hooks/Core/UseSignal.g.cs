...
[App(order:10, documentSource:"https://github.com/Ivy-Interactive/Ivy-Framework/blob/2587-rename-appattribute-path-parameter-to-group/src/Ivy.Docs.Shared/Docs/03_Hooks/02_Core/10_UseSignal.md", searchHints: ["communication", "events", "pubsub", "messaging", "broadcast", "cross-component"])]
public class UseSignalApp(bool onlyBody = false) : ViewBase
{
    public UseSignalApp() : this(false)
    {
    }
    public override object? Build()
    {
        ...
        var article = new Article().ShowToc(!onlyBody).ShowFooter(!onlyBody).Previous(appDescriptor.Previous).Next(appDescriptor.Next).DocumentSource(appDescriptor.DocumentSource).OnLinkClick(onLinkClick).Headings(new List<ArticleHeading> { new ArticleHeading("usesignal", "UseSignal", 1), new ArticleHeading("basic-usage", "Basic Usage", 2), new ArticleHeading("signal-communication-patterns", "Signal Communication Patterns", 2), new ArticleHeading("one-to-many-communication", "One-to-Many Communication", 3), new ArticleHeading("request-response-pattern", "Request-Response Pattern", 3), new ArticleHeading("key-types", "Key Types", 3), new ArticleHeading("signal-operations", "Signal Operations", 3), new ArticleHeading("signal-types", "Signal Types", 3), new ArticleHeading("broadcast-types", "Broadcast Types", 2), new ArticleHeading("session-vs-broadcast-signals", "Session vs Broadcast Signals", 3), new ArticleHeading("available-broadcast-types", "Available Broadcast Types", 3), new ArticleHeading("see-also", "See Also", 2), })
            | new global::Ivy.Docs.Shared.Internal.SmartSearchView()
            | new Markdown("# UseSignal").OnLinkClick(onLinkClick)
            | Lead(
                """"
                Signals enable inter-component communication in Ivy [applications](app://onboarding/concepts/apps), allowing components to send and receive messages across the component tree.
            ...
            | (Vertical() 
                | new CodeBlock(
                    ...
                    """",Languages.Csharp)
                | new Box().Content(new SignalExample())
            )
            | new Markdown(
                """"
            ...
            | new Markdown(
                ...
                """").OnLinkClick(onLinkClick)
            | Tabs( 
                new Tab("Demo", new Box().Content(new RequestResponseDemo())),
                new Tab("Code", new CodeBlock(
                    """"
            ...
            | new Markdown(
                ...
                **Getting a signal** (for sending or receiving):
                """").OnLinkClick(onLinkClick)
            | new CodeBlock("var signal = UseSignal<CounterSignal, int, string>();",Languages.Csharp)
            | new Markdown("**Sending messages**:").OnLinkClick(onLinkClick)
            | new CodeBlock(
                """"
                var signal = UseSignal<CounterSignal, int, string>();
                var results = await signal.Send(42); // Returns TOutput[] from all subscribers
                """",Languages.Csharp)
            | new Markdown("**Receiving messages**:").OnLinkClick(onLinkClick)
            | new CodeBlock(
                """"
                var signal = UseSignal<CounterSignal, int, string>();
                UseEffect(() => signal.Receive(input => {
            ...
            | new Markdown(
                """"
                **Important**: Always use `UseEffect()` to manage signal subscriptions for proper [lifecycle handling](app://onboarding/concepts/views).
            ...
            | new Markdown(
                ...
                - **`Chrome`** - Parent [Chrome](app://onboarding/concepts/chrome) session (for embedded apps)
                """").OnLinkClick(onLinkClick)
            | new CodeBlock(
                """"
                [Signal(BroadcastType.App)]
        ...
        Type[] _ = [typeof(Onboarding.Concepts.AppsApp), typeof(Onboarding.Concepts.ViewsApp), typeof(Onboarding.Concepts.ChromeApp), typeof(Hooks.Core.UseStateApp), typeof(Hooks.Core.UseEffectApp)]; 
        return article;
    }
}
