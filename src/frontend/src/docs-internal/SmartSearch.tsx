...
export const SmartSearch: React.FC<SmartSearchProps> = ({
      ...
      buttons.forEach((btn, i) => {
        btn.onmouseenter = () => {
          ...
          buttons.forEach((b, j) => {
            ...
            b.classList.toggle("text-accent-foreground", selectedIndexRef.current === j);
          });
        };
        btn.classList.toggle("bg-accent", i === selectedIndexRef.current);
        btn.classList.toggle("text-accent-foreground", i === selectedIndexRef.current);
      });
  ...
  useEffect(() => {
    const onKeyDownCapture = (e: KeyboardEvent) => {
      ...
      const isSearchInput = e.target instanceof HTMLInputElement && e.target.type === "search";
      ...
      if (buttons.length === 0 && e.key === "Enter") {
        if (isSearchInput) {
          ...
          overlayRoot.querySelector<HTMLElement>('[data-testid="docs-smart-search-ask"]')?.click();
        }
        return;
      }
      ...
      if (e.key === "ArrowDown" || (e.key === "Tab" && !e.shiftKey)) {
        e.preventDefault();
        e.stopPropagation();
        selectedIndexRef.current = Math.min(selectedIndexRef.current + 1, buttons.length - 1);
      ...
      if (!isSearchInput && isDialogContent && document.activeElement instanceof HTMLInputElement) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("ivy-docs-open-smart-search", openSmartSearchOverlay);

    return () => {
      document.body.removeEventListener("mousedown", handleMouseDown, true);
      document.body.removeEventListener("focus", handleFocus, true);
      window.removeEventListener("ivy-docs-open-smart-search", openSmartSearchOverlay);
    };
  }, []);
