...
[App(order:1, documentSource:"https://github.com/Ivy-Interactive/Ivy-Framework/blob/2587-rename-appattribute-path-parameter-to-group/src/Ivy.Docs.Shared/Docs/02_Widgets/07_Advanced/01_DataTable.md", searchHints: ["table", "grid", "data", "rows", "columns", "sort", "filter", "search", "dataset"])]
public class DataTableApp(bool onlyBody = false) : ViewBase
{
    public DataTableApp() : this(false)
    {
    }
    public override object? Build()
    {
        ...
        var article = new Article().ShowToc(!onlyBody).ShowFooter(!onlyBody).Previous(appDescriptor.Previous).Next(appDescriptor.Next).DocumentSource(appDescriptor.DocumentSource).OnLinkClick(onLinkClick).Headings(new List<ArticleHeading> { new ArticleHeading("datatable", "DataTable", 1), new ArticleHeading("basic-usage", "Basic Usage", 2), new ArticleHeading("column-configuration", "Column Configuration", 2), new ArticleHeading("advanced-configuration", "Advanced Configuration", 2), new ArticleHeading("row-actions", "Row Actions", 2), new ArticleHeading("cell-click-events", "Cell Click Events", 2), new ArticleHeading("triggering-refreshes", "Triggering Refreshes", 2), new ArticleHeading("ai-powered-filtering", "AI-Powered Filtering", 2), new ArticleHeading("enabling-ai-filtering", "Enabling AI Filtering", 3), new ArticleHeading("natural-language-queries", "Natural Language Queries", 3), new ArticleHeading("smart-interpretation", "Smart Interpretation", 3), new ArticleHeading("supported-filter-grammar", "Supported Filter Grammar", 3), new ArticleHeading("datetime-filtering", "DateTime Filtering", 2), new ArticleHeading("performance-with-large-datasets", "Performance with Large Datasets", 2), new ArticleHeading("faq", "Faq", 2), new ArticleHeading("api", "API", 2), })
            | new global::Ivy.Docs.Shared.Internal.SmartSearchView()
            | new Markdown("# DataTable").OnLinkClick(onLinkClick)
            | Lead("Display and interact with large datasets using high-performance data tables with sorting, filtering, [pagination](app://widgets/common/pagination), and real-time updates powered by Apache Arrow.")
            | new Markdown(
                """"
                The `DataTable` [widget](app://onboarding/concepts/widgets) provides a powerful, high-performance solution for displaying tabular data. Use it inside [views](app://onboarding/concepts/views) and [layouts](app://onboarding/concepts/layout), with [state](app://hooks/core/use-state) for dynamic data or row actions. Built on Apache Arrow for optimal performance with large datasets, it supports automatic type detection, sorting, filtering, column grouping, and customization.
            ...
            | Tabs( 
                new Tab("Demo", new Box().Content(sampleUsers.ToDataTable()
    ...
    .Height(Size.Units(100)))),
            ...
            | new Markdown(
                ...
                **Column customization methods:**
                ...
                - **Width** - Set column width using [Size](app://api-reference/ivy/size) (e.g. `Size.Px()`, `Size.Percent()`).
                - **Align** - Control text alignment ([Align](app://api-reference/ivy/align): Left, Right, Center)
    ...
    .Height(Size.Units(100)))),
            ...
            | new Markdown(
                ...
                **Configuration options:**
                ...
                Add contextual actions to each row using `RowActions()` and handle them via `OnRowAction()`. Actions are rendered as icons or [buttons](app://widgets/common/button) that appear when hovering over a row. Row actions support both simple menu items and nested [dropdown menus](app://widgets/common/drop-down-menu).
                ...
                """").OnLinkClick(onLinkClick)
            | Tabs( 
                new Tab("Demo", new Box().Content(new RowActionsDemo())),
                new Tab("Code", new CodeBlock(
                    """"
            ...
            | new Callout("Use Renderer(expr, new LinkDisplayRenderer { Type = LinkDisplayType.Url }) to mark a URL string column as a clickable hyperlink. Click on a link to open it. External links (http/https) open in a new focused tab, while relative URLs navigate in the same tab.", icon:Icons.Info).OnLinkClick(onLinkClick)
            | new Markdown(
                """"
                ## Cell Click Events
                
                Enable single- and double-click handlers for any cell by setting `EnableCellClickEvents = true` in the table configuration and wiring up `.OnCellClick()` and `.OnCellActivated()` delegates. See [event handling](app://onboarding/concepts/event-handlers) for patterns.
            ...
            | new CodeBlock(
                ...
                """",Languages.Text)
            | new Markdown("into the filter box (Ctrl/Cmd&nbsp;+&nbsp;F) or the column filter UI, and the underlying dataset will be queried server-side. The parsing engine recognises dates without needing quotes if there are no spaces, but quoting is recommended.").OnLinkClick(onLinkClick)
            | Tabs( 
                new Tab("Demo", new Box().Content(new DateFilterDemo())),
                new Tab("Code", new CodeBlock(
                    """"
            ...
            | new Markdown(
                ...
                """").OnLinkClick(onLinkClick)
            | Tabs( 
                new Tab("Demo", new Box().Content(Enumerable.Range(1, 500)
    ...
    .Height(Size.Units(100)))),
            ...
            | new Expandable("How do I handle row actions on a DataTable?",
                Vertical().Gap(4)
                | new Markdown(
                    """"
            ...
            | new Expandable("How do I show navigation properties in a DataTable?",
                Vertical().Gap(4)
                | new Markdown(
                    """"
                ...
                | new CodeBlock(
                    ...
                    """",Languages.Csharp)
                | new Markdown("This also simplifies the DataTable configuration since you don't need to `.Hidden()` navigation properties or other fields you don't want displayed.").OnLinkClick(onLinkClick)
            )
            | new Expandable("How do I display dictionary or dynamic data in a DataTable?",
                Vertical().Gap(4)
                | new Markdown(
                    """"
                ...
                | new CodeBlock(
                    ...
                    """",Languages.Csharp)
                | new Markdown("If columns are truly dynamic (unknown at compile time), consider building a `List<Dictionary<string, object>>` and using `.ToTable()` with explicit column definitions instead.").OnLinkClick(onLinkClick)
            )
            | new Expandable("How do I handle row clicks in DataTable?",
                Vertical().Gap(4)
                | new Markdown("There is no `OnRowClick` method. Use `OnCellClick` to handle individual cell clicks:").OnLinkClick(onLinkClick)
                | new CodeBlock(
                    """"
                    items.ToDataTable()
                        .OnCellClick(async e =>
                    ...
                    """",Languages.Csharp)
                | new Markdown("For row-level action buttons (edit, delete), use the **fluent API** with **enum tags** and <c>Enum.TryParse</c>:").OnLinkClick(onLinkClick)
                | new CodeBlock(
                    """"
                    private enum RowAction { Edit, Delete }
            ...
            | new WidgetDocsView("Ivy.DataTable", "Ivy.DataTableWidgetExtensions", "https://github.com/Ivy-Interactive/Ivy-Framework/blob/main/src/Ivy/Widgets/DataTables/DataTable.cs")
            ;
        // Build errors here indicates that one or more referenced apps don't exist. Check markdown links.
        Type[] _ = [typeof(Widgets.Common.PaginationApp), typeof(Onboarding.Concepts.WidgetsApp), typeof(Onboarding.Concepts.ViewsApp), typeof(Onboarding.Concepts.LayoutApp), typeof(Hooks.Core.UseStateApp), typeof(ApiReference.Ivy.SizeApp), typeof(ApiReference.Ivy.AlignApp), typeof(Widgets.Common.ButtonApp), typeof(Widgets.Common.DropDownMenuApp), typeof(Onboarding.Concepts.EventHandlersApp)]; 
        return article;
    }
}
